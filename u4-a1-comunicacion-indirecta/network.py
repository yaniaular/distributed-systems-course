...
class MulticastReceiver:
    ...
    def listen_forever(incoming_queue, address, maximum_connections):
        ...
        # 2. Permitir la reutilización de la dirección (para reiniciar el receptor sin error)
        sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)

        # 3. Enlazar (bind) el socket a todas las interfaces locales, en el puerto indicado
        sock.bind(('', port))

        # 4. Convertir la dirección multicast a formato binario y unirse al grupo
        group_bin = socket.inet_aton(multicast_group)
        mreq = struct.pack('4sL', group_bin, socket.INADDR_ANY)
        sock.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, mreq)
        print(f"[Receptor] Escuchando en grupo: {multicast_group}, puerto: {port}")
        try:
            while True:
